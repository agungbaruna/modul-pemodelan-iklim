#' Function for importing the WRF output data into rast class
#' using `terra` package
#' 
wrf.raster <- function(wrf.file, 
  ...
  if (nlev == 1){
    ...
    w.r   <- rast(nrows = length(XLAT), ncols = length(XLONG), 
                  nlyrs = length(XTIME),
                  xmin = min(XLONG), xmax = max(XLONG), 
                  ymin = min(XLAT), ymax = max(XLAT), 
    ...
    w.r    <- brick(nrows = length(XLAT), ncols = length(XLONG), 
                    nlyrs = length(XTIME),
                    xmin = min(XLONG), xmax = max(XLONG), 
                    ymin = min(XLAT), ymax = max(XLAT), 
                    crs = "+proj=longlat +datum=WGS84 +no_defs")
  ...
  # Assign time format to layer names
  names(w.r) <- paste0('Sim. +', XTIME / 60, " hour(s)")
